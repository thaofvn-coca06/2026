...
- Primary author can be any contributor. Every `git commit` should have
  `--author="Your Name <your.email@domain>"`. The harness respects the Claude
...
- **Skill names are public surface.** Renaming a skill is a major version bump and
  ...
  table, and every `gh-<name>` cross-reference in sibling SKILL.md files.
...
- Other root-level references shared across skills:
  `references/github-api-patterns.md` and `references/markdown-templates.md`.
...
- **GitHub API layer:** `lib/github_client.py`. Verified methods:
  `create_issue()`, `create_discussion()`, `create_release()`,
  `create_pull_request()`, `fetch_repository()`, `list_issues()`. Skills should
  call these rather than inline GitHub API calls.
- **Authentication:** GitHub token via `GITHUB_TOKEN` environment variable (with
  `.env` file fallback for local testing).
...
- `.agents/plugins/marketplace.json` points to `.codex-marketplace/github-skills`.
...
- `testing/` is the local scratch directory: API responses, validation reports,
  integration scripts.
...
```bash
python3 -c "from lib import GitHubClient; print('OK')"
python3 scripts/sync_codex_marketplace.py
