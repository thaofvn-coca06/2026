...
- Primary author **must** be the main contributor. Every `git commit` should have
  `--author="Your Name <your.email@domain>"`. Verify with
...
- **Skill names are public surface.** Renaming a skill is a major version bump and
  ...
  bundle list, README skill table, and every `gh-<name>` cross-reference in
...
- Other root-level references: `references/github-api-patterns.md` and
...
- **GitHub API layer:** `lib/github_client.py`. Verified methods for issues,
  discussions, PRs, releases. Skills call `lib.create_issue()`, `lib.create_discussion()`,
  `lib.create_release()`, etc.
...
- `testing/` is the local scratch directory: API responses, validation reports.
  Never write secrets above `testing/` (the rest of the repo is public). The
  `.gitignore` rule for `testing/` is load-bearing.
